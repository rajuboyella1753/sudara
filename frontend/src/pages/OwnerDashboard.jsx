    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col font-sans overflow-x-hidden text-sm">
      ...
      {isEditingProfile && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#0f172a] w-full max-w-md p-6 md:p-8 rounded-[2.5rem] border border-indigo-500/20 shadow-2xl">
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="flex flex-col items-center gap-3 p-4 border border-dashed border-indigo-500/20 rounded-2xl bg-black/20">
                  {profileForm.hotelImage ? (
                    <img src={profileForm.hotelImage} className="w-24 h-24 rounded-2xl object-cover border-2 border-blue-500" alt="Hotel" />
                  ) : (
      ...
      <nav className="bg-[#0f172a]/90 backdrop-blur-xl border-b border-indigo-500/20 px-4 md:px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 z-50 sticky top-0">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <aside className="w-full lg:w-[380px] bg-[#020617] p-4 md:p-6 flex flex-col gap-6 border-b lg:border-r border-indigo-500/20 overflow-y-auto shrink-0 scrollbar-hide">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 bg-[#020617] scroll-smooth">
          <div className="mb-8 flex flex-col gap-6 sticky top-0 bg-[#020617] z-40 pb-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 className="text-2xl md:text-4xl font-black italic uppercase tracking-tighter text-white">Kitchen Menu</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
            {filteredItems.map(i => (
              <div key={i._id} className={`group bg-[#0f172a] p-4 md:p-5 rounded-[2.5rem] border border-white/5 flex flex-col gap-5 transition-all hover:border-blue-500/30 ${!i.isAvailable && 'opacity-40 grayscale'}`}>
