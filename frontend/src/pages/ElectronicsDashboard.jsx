...
export default function ElectronicsDashboard() {
  ...
  const handleAddGadget = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Failed to add gadget.");
    } finally {
      setLoading(false);
    }
  };
...
const downloadQRCode = async () => {
    try {
      // 1. డొమైన్ మరియు ఐడీ ద్వారా నేరుగా QR కోడ్ డేటా URL ని జనరేట్ చేస్తున్నాం
      ...
      // బ్యాక్‌గ్రౌండ్ కలర్స్ & గ్రాడియంట్
      ...
      ctx.fillText("DIGITAL STOCK & QUOTE HUB", canvas.width / 2, 320);
      ...
      // 2. జనరేట్ అయిన QR ఇమేజ్ ని కాన్వాస్‌పై డ్రా చేయడం
      ...
      qrImg.onload = () => {
        ...
        ctx.fillText("HOW TO BUY / REQUEST QUOTE", canvas.width / 2, 1260); 

        const pathText = "SCAN ➔ BROWSE STOCK ➔ CHAT/CALL ➔ GET QUOTE";
        ctx.font = "bold 28px sans-serif";
        ctx.fillStyle = "#CBD5E1";
        ctx.fillText(pathText, canvas.width / 2, 1330);

        const steps = [
            "1. Open Camera and scan code",
            "2. Check live gadgets & components catalog",
            "3. Tap to call or request price quote directly"
        ];
        ...
        ctx.fillText("💡 Visit our physical store or check online inventory anytime!", canvas.width / 2, 1725);
        ...
        link.download = `${owner?.name || "Store"}_Poster.png`;
  ...
  const handleDeleteProduct = async (id) => {
    if (!window.confirm("Remove this gadget from catalog?")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col justify-between selection:bg-indigo-100">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[8px] sm:text-[10px] font-extrabold uppercase text-indigo-600 tracking-widest truncate">Gadgets Hub</p>
      </div>
    </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {/* QR Poster Button in Navbar */}
          {/* <button 
            onClick={downloadQRCode}
            className="p-2 sm:p-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl transition-all shrink-0 border border-indigo-100 flex items-center gap-1"
            title="Download QR Poster"
          >
            <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="text-[9px] font-black uppercase hidden md:inline">QR Poster</span>
          </button> */}
        ...
        {activeTab === "inventory" && (
          <>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 sm:p-6 rounded-3xl border border-slate-200/80 shadow-xs">
              <div>
                <h2 className="text-lg sm:text-xl font-black uppercase tracking-tight text-slate-900">Inventory Catalog</h2>
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Manage gadgets, pricing, and stock visibility seamlessly</p>
              </div>
              <button 
                onClick={() => setIsAddModal(true)}
                className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-5 sm:px-6 py-3.5 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-md shadow-indigo-100 transition-all flex items-center justify-center gap-2 active:scale-95 shrink-0"
              >
            ...
            <div className="w-full overflow-x-auto pb-2 scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0">
              <div className="flex items-center gap-2.5 w-max">
                {[
                  { label: "All Products", icon: Package, key: "All" },
                  { label: "Mobiles", icon: Smartphone, key: "Mobiles" },
                  { label: "Laptops", icon: Laptop, key: "Laptops" },
                  { label: "Audio", icon: Headphones, key: "Audio" },
                  { label: "Accessories", icon: Watch, key: "Accessories" }
                ].map((tab) => {
                  const Icon = tab.icon;
                  return (
                    <button
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white border border-dashed border-slate-200 rounded-3xl p-12 sm:p-16 text-center space-y-3">
                ...
                <h3 className="text-sm font-black uppercase text-slate-700 tracking-wider">No Gadgets Found</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase">Click 'Add New Gadget' to list items into your store.</p>
              </div>
            ) : (
      ...
      <footer className="bg-white border-t border-slate-200 px-4 sm:px-6 lg:px-12 py-6 mt-12">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3 text-center sm:text-left">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Sudara Electronics Hub &copy; 2026 • Secure Merchant Node
          </p>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-white w-full max-w-lg p-6 sm:p-8 rounded-[2.5xl] shadow-2xl relative border border-slate-200 max-h-[85vh] overflow-y-auto scrollbar-custom" > 
              ...
              <form onSubmit={handleUpdateSettings} className="space-y-4">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-3">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Login Credentials</h4>
                  ...
                  <div className="relative border border-dashed border-slate-300 rounded-xl p-4 text-center bg-slate-50 hover:bg-slate-100/50 transition-all cursor-pointer">
                    ...
                    {storeImagePreview ? (
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-indigo-600 uppercase">Change Image ✅</span>
                      </div>
                    ) : (
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Store GPS Coordinates</h4>
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div>
                  ...
                  <div className="relative border border-dashed border-slate-300 rounded-xl p-3 text-center bg-slate-50 cursor-pointer">
                    ...
                    />
                    <span className="text-[10px] font-black text-indigo-600 uppercase">+ Upload Interior Photos</span>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-xs">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-md p-6 sm:p-8 rounded-3xl shadow-xl relative border border-slate-200 max-h-[90vh] overflow-y-auto">
              ...
              <div className="mb-5 sm:mb-6">
                <h3 className="text-base sm:text-lg font-black uppercase text-slate-900 tracking-tight">Add New Gadget</h3>
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">List a new item to your store catalog</p>
              </div>

              <form onSubmit={handleAddGadget} className="space-y-4">
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Gadget Name</label>
                  <input type="text" required placeholder="e.g. MacBook Pro M3 / iPhone 17" value={newGadget.name} onChange={(e)=>setNewGadget({...newGadget, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl font-bold text-xs outline-none focus:border-indigo-600" />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Category</label>
                    <select value={newGadget.subCategory} onChange={(e)=>setNewGadget({...newGadget, subCategory: e.target.value})} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl font-bold text-xs outline-none focus:border-indigo-600 cursor-pointer">
                  ...
                  <div>
                    ...
                    <input type="number" required placeholder="89990" value={newGadget.price} onChange={(e)=>setNewGadget({...newGadget, price: e.target.value})} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl font-bold text-xs outline-none focus:border-indigo-600" />
                  </div>
                </div>
                  ...
                  <div className="relative border border-dashed border-slate-300 rounded-xl p-4 text-center bg-slate-50 hover:bg-slate-100/50 transition-all cursor-pointer">
                    ...
                    {imagePreview ? (
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-indigo-600 uppercase">Image Selected ✅</span>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-1">
                ...
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Description & Specifications</label>
                  <textarea placeholder="Write technical specs..." value={newGadget.description} onChange={(e)=>setNewGadget({...newGadget, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold text-xs outline-none focus:border-indigo-600 h-20 resize-none"></textarea>
                </div>
        ...
        {isEditModal && editingItem && (
          <div className="fixed inset-0 z-100 flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-xs">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-md p-6 sm:p-8 rounded-3xl shadow-xl relative border border-slate-200 max-h-[90vh] overflow-y-auto">
              ...
              <div className="mb-5 sm:mb-6">
                <h3 className="text-base sm:text-lg font-black uppercase text-slate-900 tracking-tight">Edit Product</h3>
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Update gadget specifications</p>
              </div>
            ...
            >
              <div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <Package className="w-4 h-4 text-indigo-600" /> Inventory Management
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 rounded-2xl font-black uppercase text-xs tracking-wider transition-all border border-slate-100 text-slate-700"
                  >
                    <Settings className="w-4 h-4 text-indigo-600" /> Login Details & Certificate
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 rounded-2xl font-black uppercase text-xs tracking-wider transition-all border border-slate-100 text-slate-700"
                  >
                    <Download className="w-4 h-4 text-indigo-600" /> Download QR Poster
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 rounded-2xl font-black uppercase text-xs tracking-wider transition-all border border-slate-100 text-slate-700"
                  >
                    <Settings className="w-4 h-4 text-indigo-600" /> Hub Settings
                  </button>
                </div>
              </div>

              <div className="pt-6 border-t border-slate-100 text-center">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Sudara Electronics Hub Node</p>
              </div>
            </motion.div>
          </div>
        )}
